...
def qmean(data):
    ...
    s = np.float256(0)
    for i in range(ld):
        s = np.float256(s +np.multiply(data[i],data[i]), dtype=np.float256)
        #print(f"multiplication: {i}")
        #print(f"{data[i]}*{data[i]}={data[i]*data[i]} ---> s={s}\n")
    return np.float256(np.sqrt(s/ld))
